...
from data.coco_dataset import CustomDataset
...
class CustomDataLoader(TransparentDataLoader):
    NOT_DEFINED = -1

    def __init__(self, YOUR_CUSTOM_DATASET,
                    array_of_init_seeds,
        ...
        self.YOUR_CUSTOM_DATASET = YOUR_CUSTOM_DATASET
        ...
        self.num_procs = num_procs
        self.num_batches = CustomDataLoader.NOT_DEFINED
        self.batch_it = []
        self.image_idx_x = []
        self.caption_y = []
        ...
        if self.YOUR_CUSTOM_DATASET.use_images_instead_of_features:
            print("Warning: using Images instead of features in the DataLoader")

            if self.YOUR_CUSTOM_DATASET.preproc_images_hdf5_filepath is not None:
                self.hdf5_img_file = h5py.File(self.YOUR_CUSTOM_DATASET.preproc_images_hdf5_filepath, 'r', rdcc_nbytes=0)
            ...
            self.hdf5_file = h5py.File(self.YOUR_CUSTOM_DATASET.precalc_features_hdf5_filepath, 'r', rdcc_nbytes=0)
    ...
    def init_epoch(self, epoch_it, verbose=False):
        ...
        if self.dataloader_mode == 'caption_wise':
            ...
            for img_idx in range(self.YOUR_CUSTOM_DATASET.train_num_images):
                num_captions = len(self.YOUR_CUSTOM_DATASET.train_list[img_idx]['captions'])
            ...
            while i < len(img_idx_caption_id_pair_list):
                for idx_proc in range(self.num_procs):
                    ...
                    preprocessed_caption = self.preprocess(self.YOUR_CUSTOM_DATASET.train_list[img_idx]['captions'][caption_id])
            ...
            img_idxes_list = list(range(self.YOUR_CUSTOM_DATASET.train_num_images))
    ...
    def get_next_batch(self, verbose=False, get_also_image_idxes=False, get_also_image_path=False):
        ...
        for i in range(len(img_idx_batch)):
            ...
            img_id_batch.append(self.YOUR_CUSTOM_DATASET.train_list[img_idx]['img_id'])
        ...
        if self.dataloader_mode == 'caption_wise':
            ...
            batch_caption_y_encoded = language_utils. \
                convert_allsentences_word2idx(batch_caption_y_as_string,
                                            self.YOUR_CUSTOM_DATASET.caption_word2idx_dict)
            batch_y, batch_y_num_pads = self.add_pad_according_to_batch(batch_caption_y_encoded,
                                                                        self.YOUR_CUSTOM_DATASET.caption_word2idx_dict['PAD'])
            batch_y = torch.tensor(batch_y)
        else:  # image_wise
            batch_y = [self.YOUR_CUSTOM_DATASET.train_list[img_idx]['captions'] for img_idx in img_idx_batch]
        ...
        if get_also_image_path:
            for i in range(len(img_idx_batch)):
                idx = img_idx_batch[i]
                file_path_batch_x.append(self.YOUR_CUSTOM_DATASET.train_list[idx]['img_path'])
    ...
    def get_batch_samples(self, dataset_split, img_idx_batch_list):
        ...
        for i in range(len(img_idx_batch_list)):
            ...
            if dataset_split == CustomDataset.TestSet_ID:
                caption_id = random.randint(a=0, b=len(self.YOUR_CUSTOM_DATASET.test_list[img_idx]['captions'])-1)
                caption = self.YOUR_CUSTOM_DATASET.test_list[img_idx]['captions'][caption_id]
            elif dataset_split == CustomDataset.ValidationSet_ID:
                caption_id = random.randint(a=0, b=len(self.YOUR_CUSTOM_DATASET.val_list[img_idx]['captions'])-1)
                caption = self.YOUR_CUSTOM_DATASET.val_list[img_idx]['captions'][caption_id]
            else:
                caption_id = random.randint(a=0, b=len(self.YOUR_CUSTOM_DATASET.train_list[img_idx]['captions'])-1)
                caption = self.YOUR_CUSTOM_DATASET.train_list[img_idx]['captions'][caption_id]

            preprocessed_caption = self.preprocess(caption)

        if dataset_split == CustomDataset.TestSet_ID:
            batch_captions_y_as_string.append(preprocessed_caption)
            img_id_batch.append(self.YOUR_CUSTOM_DATASET.test_list[img_idx]['img_id'])
        elif dataset_split == CustomDataset.ValidationSet_ID:
            batch_captions_y_as_string.append(preprocessed_caption)
            img_id_batch.append(self.YOUR_CUSTOM_DATASET.val_list[img_idx]['img_id'])
        else:
            batch_captions_y_as_string.append(preprocessed_caption)
            img_id_batch.append(self.YOUR_CUSTOM_DATASET.train_list[img_idx]['img_id'])
        ...
        batch_caption_y_encoded = language_utils. \
            convert_allsentences_word2idx(batch_captions_y_as_string,
                                        self.YOUR_CUSTOM_DATASET.caption_word2idx_dict)
        batch_y, batch_y_num_pads = self.add_pad_according_to_batch(batch_caption_y_encoded,
                                                                    self.YOUR_CUSTOM_DATASET.get_pad_token_idx())
        batch_y = torch.tensor(batch_y)
    ...
    def get_PADDED_image_batch_by_idx(self, img_idx_list, verbose=False):
        ...
        for img_idx in img_idx_list:
            if self.YOUR_CUSTOM_DATASET.preproc_images_hdf5_filepath is not None:
                ...
                img_id = self.YOUR_CUSTOM_DATASET.train_list[img_idx]['img_id']
                np_array = self.hdf5_img_file[str(img_id) + '_img'][()]
                list_of_images.append(torch.tensor(np_array, dtype=torch.float32).unsqueeze(0))
            else:
                file_path = self.YOUR_CUSTOM_DATASET.train_list[img_idx]['img_path']
    ...
    def get_images_by_idx(self, img_idx, dataset_split, transf_mode='train', get_also_id=False):
        if dataset_split == CustomDataset.TestSet_ID:
            file_path = self.YOUR_CUSTOM_DATASET.test_list[img_idx]['img_path']
            img_id = self.YOUR_CUSTOM_DATASET.test_list[img_idx]['img_id']
        elif dataset_split == CustomDataset.ValidationSet_ID:
            file_path = self.YOUR_CUSTOM_DATASET.val_list[img_idx]['img_path']
            img_id = self.YOUR_CUSTOM_DATASET.val_list[img_idx]['img_id']
        else:
            file_path = self.YOUR_CUSTOM_DATASET.train_list[img_idx]['img_path']
            img_id = self.YOUR_CUSTOM_DATASET.train_list[img_idx]['img_id']
    ...
    def get_all_image_captions_by_idx(self, img_idx, dataset_split):
        if dataset_split == CustomDataset.TestSet_ID:
            caption_list = self.YOUR_CUSTOM_DATASET.test_list[img_idx]['captions']
        elif dataset_split == CustomDataset.ValidationSet_ID:
            caption_list = self.YOUR_CUSTOM_DATASET.val_list[img_idx]['captions']
        else:
            caption_list = self.YOUR_CUSTOM_DATASET.train_list[img_idx]['captions']
    ...
    def preprocess(self, caption):
        ...
        caption = [self.YOUR_CUSTOM_DATASET.get_sos_token_str()] + \
                language_utils.tokenize(caption)[0] + [self.YOUR_CUSTOM_DATASET.get_eos_token_str()]
        preprocessed_tokenized_caption = []
        for word in caption:
            if word not in self.YOUR_CUSTOM_DATASET.caption_word2idx_dict.keys():
                preprocessed_tokenized_caption.append(self.YOUR_CUSTOM_DATASET.get_unk_token_str())
            else:
